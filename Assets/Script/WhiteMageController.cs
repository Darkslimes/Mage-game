...
public class WhiteMageController : MonoBehaviour
{
    ...
    void Update()
    {
        if(screenTargets.Count > 0 && !casting)
        {
            targetCount = targetIndex();
            target = screenTargets[targetCount];
        }
        ...
        if (Input.GetKeyDown("x"))
        {
            if (fastSkillrefresh[5] == false)
                StartCoroutine(FastPlay(5, 2f, 2.7f));
        }
        ...
        if (Input.GetMouseButton(0) && aim.enabled == true && activeTarger)
        {
            ...
            secondLayerWeight = Mathf.Lerp(secondLayerWeight, 1, Time.deltaTime * 20);
            if (fireCountdown <= 0f)
            {
                GameObject projectile = Instantiate(PrefabsCast[8], rightFirePoint.position, rightFirePoint.rotation);
        ...
        {
            secondLayerWeight = Mathf.Lerp(secondLayerWeight, 0, Time.deltaTime * 20);
        }
    ...
    public IEnumerator FastPlay(int EffectNumber, float castDelay, float endDelay)
    {
        ...
        if (EffectNumber == 2 || EffectNumber == 5 || EffectNumber == 7)
        {
            ...
            StartCoroutine(cameraShaker.Shake(0.4f, 5, 1f, 1.19f));
        ...
        if (TargetMarker2 && casting == false)
        {
            ...
            while (true)
            {
                ...
                if (Input.GetMouseButtonDown(0) && casting == false)
                {
                    ...
                    StartCoroutine(cameraShaker.Shake(0.4f, 7, 0.45f, 0.55f));
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            screenTargets.Add(other.transform);
        }
    }
    ...
    private void OnTriggerExit(Collider other)
    {
        if (other.CompareTag("Enemy"))
        {
            if (screenTargets.Contains(other.transform))
                screenTargets.Remove(other.transform);
        }
    }
}
